		...
		if (currentMaps.currentMapIs("HelmsDeep")) {
			...
			if (FlagRadius.isPlayerInRadius(p, flag)) {
				...
				if (flag.equalsIgnoreCase("Horn")) {
					...
					if (FlagRadius.Horn1.contains(p) || FlagRadius.Horn2.contains(p)) {

						if (FlagRadius.Horn1.size() == 1 || FlagRadius.Horn2.size() == 1) {

							if (FlagRadius.ThisTeamIsCapturing(1, flag)) {

								for (Player online : FlagRadius.Horn1) {
									online.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(ChatColor.DARK_AQUA + "+1 flagcapping point(s)" + ChatColor.AQUA + " Flag: The Horn"));
									playSound(online);
							...
							if (FlagRadius.ThisTeamIsCapturing(2, flag)) {
								...
								for (Player online : FlagRadius.Horn2) {
									online.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(ChatColor.DARK_AQUA + "+1 flagcapping point(s)" + ChatColor.AQUA + " Flag: The Horn"));
									playSound(online);
									MVPstats.setCaptures(online.getUniqueId(), MVPstats.getCaptures(online.getUniqueId()) + 1);
						...
						if (FlagRadius.Horn1.size() == 2 || FlagRadius.Horn2.size() == 2) {

							if (FlagRadius.ThisTeamIsCapturing(1, flag)) {

								for (Player online : FlagRadius.Horn1) {
									online.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(ChatColor.DARK_AQUA + "+2 flagcapping point(s) [Team Bonus]" + ChatColor.AQUA + " Flag: The Horn"));
									playSound(online);
						...
						if (FlagRadius.Horn1.size() == 3 || FlagRadius.Horn2.size() == 3) {

							if (FlagRadius.ThisTeamIsCapturing(1, flag)) {

								for (Player online : FlagRadius.Horn1) {
									online.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(ChatColor.DARK_AQUA + "+3 flagcapping point(s) [Team Bonus]" + ChatColor.AQUA + " Flag: The Horn"));
									playSound(online);
						...
						if (FlagRadius.Horn1.size() >= 4 || FlagRadius.Horn2.size() >= 4) {

							if (FlagRadius.ThisTeamIsCapturing(1, flag)) {

								for (Player online : FlagRadius.Horn1) {
									online.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(ChatColor.DARK_AQUA + "+4 flagcapping point(s) [Team Bonus]" + ChatColor.AQUA + " Flag: The Horn"));
									playSound(online);
					...
					if (FlagRadius.Horn1.contains(p) || FlagRadius.Horn2.contains(p)) {
						...
						if (FlagRadius.Horn1.size() == 1 || FlagRadius.Horn2.size() == 1) {
							...
							if (FlagRadius.ThisTeamIsCapturing(1, flag)) {
								...
								for (Player online : FlagRadius.Horn1) {
									online.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(ChatColor.GOLD + "Flag fully captured!" + ChatColor.AQUA + " Flag: The Horn"));
									playSound(online);
									MVPstats.setCaptures(online.getUniqueId(), MVPstats.getCaptures(online.getUniqueId()) + 1);
									return;
								}
